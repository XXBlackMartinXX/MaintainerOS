    ...
    return (
      <div>
        <PageHeader
          ...
          description="AI-drafted documentation for the files every healthy project needs."
        />
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="panel rounded-xl p-4">
            <div className="flex items-start justify-between gap-3 mb-3 flex-wrap">
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {DOC_TYPE_LABELS[docType].purpose}
                </p>
                <p className="text-[10px] font-mono text-muted-foreground mt-0.5">
                  {DOC_TYPE_LABELS[docType].filename}
                </p>
              </div>
              <Button
                size="sm"
            ...
            {!activeDraft && !draftsQ.isLoading && (
              <div className="rounded-md border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
                No draft yet. Click{" "}
                <span className="text-foreground font-medium">Generate draft</span> to create one
                from live repository signals.
              </div>
            )}
              ...
              <>
                ...
                <div className="flex items-center gap-1 mb-2 text-xs">
                  ...
                  <span className="ml-auto text-muted-foreground">
                    {activeDraft.model ?? "—"} · updated{" "}
                    {formatDistanceToNow(new Date(activeDraft.updated_at), { addSuffix: true })}
                  </span>
                </div>

                {previewMode ? (
                  <pre className="w-full min-h-[420px] max-h-[600px] overflow-auto rounded-md border border-border bg-background p-4 text-xs leading-relaxed whitespace-pre-wrap font-mono text-foreground/90">
                    {editedBody}
                  </pre>
                ) : (
                  <textarea
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  ...
                  <Button
                    size="sm"
                    onClick={() => decisionMut.mutate("approve")}
                  ...
                  <div className="ml-auto flex items-center gap-1">
                    <FutureButton
                      ...
                      label="Create PR with this doc — coming soon"
                    />
                    <FutureButton
                      icon={<Github className="size-3.5" />}
                      label="Commit file to GitHub — coming soon"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
                  <InfoPanel
                    ...
                    items={structured.missingContext ?? []}
                    tone="warning"
                    empty="The draft used all available signals."
                  />
                  <InfoPanel
                    title="Safety notes"
                    items={structured.safetyNotes ?? []}
                    tone="info"
                    empty="No specific cautions flagged."
                  />
                </div>
          ...
          {drafts.length > 1 && (
            <div className="panel rounded-xl p-3">
              <div className="px-1 py-1 text-[10px] uppercase tracking-wider text-muted-foreground mb-1">
                Previous drafts
              </div>
                ...
                {drafts.map((d) => (
                  ...
                  >
                    ...
                    <span className="truncate flex-1">
                      {d.title || DOC_TYPE_LABELS[d.doc_type as DocType]?.title}
                    </span>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-md border px-1.5 py-0.5 text-[10px] font-medium capitalize ${map[status] ?? map.pending}`}
    >
      {status}
    </span>
  );
