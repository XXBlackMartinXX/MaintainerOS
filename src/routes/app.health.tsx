...
function computeScores(d: Inputs): { rows: ScoreRow[]; total: number; recommendations: string[] } {
  ...
  const releaseReadiness =
    d.pulls.filter((p) => p.merged_at && daysAgo(p.merged_at) < 60).length > 0 ? 75 : 55;

  const rows: ScoreRow[] = [
    {
      key: "Issue freshness",
      score: issueFreshness,
      weight: 0.2,
    ...
    {
      key: "Security readiness",
      score: securityReadiness,
      weight: 0.15,
      rationale:
        "Heuristic baseline. Security advisories and dependency scans are not yet wired in.",
      missing: true,
    },
    {
  ...
  if (staleIssues.length > 0)
    recommendations.push(
      `Triage ${staleIssues.length} stale issue${staleIssues.length === 1 ? "" : "s"} older than 60 days.`,
    );
  if (stalePRs.length > 0)
    recommendations.push(`Review ${stalePRs.length} stale PR${stalePRs.length === 1 ? "" : "s"}.`);
  if (labelCount < 5)
    recommendations.push(
      "Add a small core label set (bug, enhancement, docs, good-first-issue, question) to make triage faster.",
    );
  if (recentCommits === 0)
...
function HealthPage() {
  ...
  if (q.isLoading)
    return (
      <div>
        {header}
        <Loading label="Loading health inputs…" />
      </div>
    );
  if (!hasData)
    return (
      <div>
        {header}
        <EmptySyncedDataState repositoryId={selected!.id} resource="health data" />
      </div>
    );

  const total = computed!.total;
  const totalColor =
    total >= 80 ? "text-success" : total >= 60 ? "text-warning" : "text-destructive";
  ...
  return (
    <div>
      ...
      <div className="panel-elevated rounded-xl p-6 mb-4 flex items-center gap-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Computed from synced GitHub data. Some signals are partial — see the breakdown for
            details.
          </p>
        </div>
      </div>
        ...
        <div className="mt-6 panel rounded-xl p-5">
          ...
          <ul className="mt-3 space-y-2 text-sm">
            {computed!.recommendations.map((r) => (
              <li key={r} className="rounded-md border border-border bg-surface px-3 py-2">
                {r}
              </li>
            ))}
          </ul>
      ...
      <div className="mt-6 panel rounded-xl p-4 text-xs text-muted-foreground flex items-start gap-2">
        ...
        <p>
          Security readiness is a heuristic baseline only. MaintainerOS does not yet inspect
          security advisories or dependency vulnerabilities — we say <em>review recommended</em>,
          never that a repo is secure or insecure.
        </p>
      </div>
    </div>
  );
