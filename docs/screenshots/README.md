...
These screenshots are referenced from the main `README.md` and launch posts.
Capture them from a real workspace (or clearly-labeled demo mode) before
publishing the repository publicly. **Do not invent or mock screenshots.**
...
- Use demo mode (`/app` with the demo banner visible) for screenshots that
  would otherwise show private user data.
- Keep the demo banner visible — do not crop it out.
- Do not show real user emails, tokens, or webhook secrets.
